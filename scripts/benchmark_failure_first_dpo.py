#!/usr/bin/env python3
"""
Run the failure-first benchmark subset for the DPO stage.
"""
...
DEFAULT_DPO_MODEL = "outputs/base_failure_dpo_qwen25_3b_v1/dpo_model_v1"
DEFAULT_OUTPUT = "outputs/base_failure_dpo_qwen25_3b_v1/benchmark_base_vs_dpo_failure_first.json"


def load_dpo_case_ids(failure_plan_path: str) -> List[str]:
    payload = json.loads(Path(failure_plan_path).read_text(encoding="utf-8"))
    return [
        record["case_id"]
        for record in payload.get("records", [])
        if record.get("proposed_action") == "dpo"
    ]


def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark the failure-first DPO subset.")
    ...
    selected_case_ids = load_dpo_case_ids(args.failure_plan)
    ...
    benchmark.run(stages=["base", "dpo"], output_path=args.output)
